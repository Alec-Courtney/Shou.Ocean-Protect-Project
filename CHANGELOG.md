...
*   **可视化总览面板**  
    *   `frontend/index.html` 新增地图顶层容器与玻璃拟态覆盖层，包含品牌标题、连接状态胶囊、四宫格核心指标（在线船只、当日预警、最高风险、当前选中）以及地图图例、快捷控制条（回中、底图切换、标签开关）。
    *   预警统计仍保留在左侧面板，但实时计数也同步到顶部指标卡片，实现“一眼洞察”。

*   **样式重塑**  
    *   `frontend/css/style.css` 引入 Glassmorphism 风格面板、指标栅格、图例及控制按钮样式，同时调整地图容器、右侧面板间距和阴影/圆角等细节，使整体视觉更统一、现代。

*   **交互与数据联动**  
    *   `frontend/js/main.js` 增强 WebSocket 连接指示、统计卡片刷新逻辑，并新增卫星/街道底图切换、视角回中、全局/局部标签开关等控件。
    *   统计卡片自动读取在线船数、最高预警等级、当前选中船名，确保 UI 与实时数据保持同步；地图控制按钮与原有标签切换逻辑合并，交互更加直观。
